<?php

namespace App\Http\Controllers\MoneyChange;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use App\Models\Client;
use App\Models\Caisse\Caisse;
use App\Models\MoneyChange\Devise;
use App\Models\TypeReglement;
use App\Models\MoneyChange\Stock;
use App\Models\MoneyChange\OperationDevise;
use App\Models\Agences\Agence;
use App\Models\Agences\DeviseAgence;
use App\Models\Caisse\MouvementCaisse;
use App\Models\Region;
use Barryvdh\DomPDF\Facade\Pdf;

class VenteDeviseController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        $id = Auth::user()->id;

        if (isset(Caisse::where('user_id', $id)->first(['id'])->id)) {
            $caisse_id = Caisse::where('user_id', $id)->first(['id'])->id;
            $caisse = Caisse::find($caisse_id);
            $agence_id = Auth::user()->agence_id;
            $agence = Agence::find($agence_id);
            $regions = Region::orderBy('nom')->where('id','!=',$agence->region_id)->get();
            $operations = OperationDevise::where('user_id', $id)->where('client_id', '!=', Null)->where('sens_operation', 'entree')->get();
            return view('money_change.vente_devise', compact('caisse', 'operations', 'agence','regions'));
        }
        return view('money_change.message');
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {

        /**
         * selection du client
         */

         $agence_id = Auth::user()->agence_id;
         // Taux achat change
        $taux_a=$request->taux_v-$request->marge;

        //Calcule de benefice sur vente change
        $benefice=($request->montant/$taux_a)-($request->montant/$request->taux_v);

        $client = Client::find($request->c_id);
        $user_id = Auth::user()->id;

        if (Caisse::where('user_id', $user_id)->first(['id'])->id) {

            $caisse_id = Caisse::where('user_id', $user_id)->first(['id'])->id;

            $montant_operation = $request->montant;

            if (isset(Stock::where('caisse_id', $caisse_id)->where('devise_id', $request->devise)->first(['id'])->id)) {

                $stock = Stock::where('caisse_id', $caisse_id)->where('devise_id', $request->devise)->first(['id', 'montant']);
                /**
                 * montant du stock
                 */
                $montant_stock = $stock->montant;

                /**
                 * montant saisie
                 */
                $montant = round($request->montant / $request->taux_v);
                /**
                 * montant total
                 */
                $montant_total = $montant_stock - $montant;
                if ($montant_stock < $montant) {

                    return redirect('/vente_devise')->with('danger', 'Le stock de la devise est insuffisant ');
                } else {

                    // dd($montant_total);
                    $compte_caisse = Caisse::where('user_id', $user_id)->first(['compte'])->compte;
                    $date_comptable = Caisse::where('user_id', $user_id)->first(['date_comptable'])->date_comptable;
                    /**
                     * mise a jour du fournisseur
                     */
                    $client->update([
                        'nom_client' => $request->nom_client,
                        'telephone' => $request->telephone,
                        'adresse' => $request->adresse,
                    ]);
                    /**
                     * enregistrement de l'operation
                     */
                    OperationDevise::create([
                        'montant_operation' => $montant_operation,
                        'sens_operation' => 'entree',
                        'client_id' => $request->c_id,
                        'devise_id' => $request->devise,
                        'taux' => $request->taux_v,
                        'coefficient' => $request->marge,
                        'benefice' => $benefice,
                        'reglement_id' => $request->reglement,
                        'date_comptable' => $date_comptable,
                        'caisse_id' => $caisse_id,
                        'user_id' => $user_id,
                    ]);

                    $agence = Agence::find($agence_id);
                    $solde_compte=$agence->compte_agence+$benefice;
                    $agence->update([
                        'compte_agence' => $solde_compte,
                    ]);


                    $compte = $compte_caisse + $montant_operation;

                    $caisse = Caisse::find($caisse_id);

                    /**
                     * mise a jour du moouvement caisse
                     */
                    $user_id = Auth::user()->id;
                    MouvementCaisse::create([
                        'caisse_id' => $caisse->id,
                        'user_id' => $user_id,
                        'description' => 'Vente change',
                        'entree' => $montant_operation,
                        'solde' => $compte,
                        'date_comptable' => $date_comptable,

                    ]);
                    /**
                     * mise a jour de la caisse
                     */
                    $caisse->update([
                        'compte' => $compte,
                    ]);
                    /**
                     * mise a jour du stock
                     */
                    $stock->update([
                        'montant' => $montant_total,
                        'devise_id' => $request->devise,
                        'caisse_id' => $caisse_id,
                    ]);

                    $id = Auth::user()->id;
                    $operation = OperationDevise::where('user_id', $id)->latest('id')->first();;
                    return redirect()->route('vente_devise.show', $operation)->with('success', 'operation effectuee avec succès');
                }
            } else {
                return redirect('/vente_devise')->with('danger', 'Le stock de la devise n\'existe pas ');
            }
        }
    }

    /**
     * Display the specified resource.
     */
    public function show(string $id)
    {
        $operation = OperationDevise::find($id);
        $agence_id = Auth::user()->agence_id;
        $agence = Agence::find($agence_id);
        return view('money_change.show_vente_devise', compact('operation', 'agence'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(string $id)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, string $id)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(string $id)
    {
        //
    }

    public function client(Request $request)
    {

        $data = $request->validate([
            'telephone' => 'required | min:8',
        ]);
        $data = $request->all();

        $tel = $data['telephone'];
        /**
         * si le telephone existe afficher le client
         */
        if (isset(client::where('telephone', $tel)->first(['id'])->id)) {
            $agence_id = Auth::user()->agence_id;
            $agence = Agence::find($agence_id);
            $client_id = client::where('telephone', $tel)->first(['id'])->id;
            $client = client::find($client_id);
            $devise_agences = DeviseAgence::where('agence_id', $agence_id)->get();
            $reglements = TypeReglement::all();
            return view('money_change.detail_vente_devise', compact('client', 'devise_agences', 'reglements', 'agence'));
            /**
             * si non enregistre le client et affiche le formulaire
             */
        } else {
            $societe_id = Auth::user()->societe_id;
            client::create([
                'telephone' => $data['telephone'],
                'societe_id'=>$societe_id,
            ]);
            /**
             * si le telephone existe afficher le client
             */
            $agence_id = Auth::user()->agence_id;
            $devise_agences = DeviseAgence::where('agence_id', $agence_id)->get();
            $client_id = client::where('telephone', $tel)->first(['id'])->id;
            $client = client::find($client_id);
            $devises = Devise::all();
            $reglements = TypeReglement::all();
            return view('money_change.detail_vente_devise', compact('client', 'devise_agences', 'reglements'));
            //return redirect('/vente_devise')->with('success','client ajouté avsec succès');
        }
    }
    public function taux_devise(Request $request)
    {
        if (Auth::check()) {
            $societe_id = Auth::user()->societe_id;
            $agence_id = Auth::user()->agence_id;

            $data['devises'] = DeviseAgence::select('devise_id', 'taux')
                ->where('devise_id', $request->id)->where('agence_id', $agence_id)->get(['devise_id', 'taux']);
            return response()->json($data);
        }
        return redirect('/auth')->with('success', "Vous n'êtes pas autorisé à accéder");
    }

    public function stock_devise(Request $request)
    {
        if (Auth::check()) {
            $societe_id = Auth::user()->societe_id;
            $user_id = Auth::user()->id;
            $caisse_id = Caisse::where('user_id', $user_id)->first(['id'])->id;

            $data['stocks'] = Stock::select('montant', 'id')
                ->where('devise_id', $request->id)->where('caisse_id', $caisse_id)->get(['montant', 'id']);
            return response()->json($data);
        }
        return redirect('/auth')->with('success', "Vous n'êtes pas autorisé à accéder");
    }

    public function print()
    {
        $id = Auth::user()->id;
        $operation = OperationDevise::where('user_id', $id)->latest('id')->first();

        $pdf = PDF::setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true])
            ->loadView('print.change.recu_vente_devise', compact('operation'));

        return $pdf->download('recu_vente.pdf');
    }

    public function region_code(Request $request)
    {

        if (Auth::check()) {
            $data['code'] = Region::where('id', $request->id)->get();
            return response()->json($data);
        }
        return redirect('/')->with('danger', "Session expirée");
    }
}
