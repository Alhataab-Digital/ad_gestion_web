...
class CommandeController extends Controller
{
    ...
    public function index()
    {
        if (Auth::check()) {
            ...
            return view('investissement.commande', compact('commandes', 'commandes_cs', 'commandes_lv', 'commandes_an'));
        }
    ...
    public function edit(string $id)
    {
        if (Auth::check()) {
            ...
            // $total_ht=DetailCommande::where('commande_id',$commande->id)->selectRaw('sum(quantite_commandee*prix_unitaire_commande) as total')->first('total');
            // return view('investissement.commande_encours', compact('commande','total_ht'));
            $agence_id = Auth::user()->agence_id;
            $produits = Produit::where('agence_id', $agence_id)->get();

            $fournisseurs = Fournisseur::all();
            return view('investissement.nouvelle_commande', compact('produits', 'fournisseurs', 'commande', 'detail_commandes', 'total_ht'));
        }
        return redirect('/')->with('danger', "Session expirée");
    }
